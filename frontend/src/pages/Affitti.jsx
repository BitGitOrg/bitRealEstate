...
const STATO_INCASSO = {
  pagato: { label: "Pagato", icon: CheckCircle2, color: "#34D399" },
  in_ritardo: { label: "In ritardo", icon: Clock, color: "#FBBF24" },
  non_pagato: { label: "Non pagato", icon: XCircle, color: "#F87171" },
  sollecitato: { label: "Sollecitato", icon: AlertCircle, color: "#FB923C" },
};
    ...
    <Layout title="Affitti & Locazioni" subtitle={`${contratti.length} contratti attivi · Tasso occupazione ${occupazione}%`}>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <SectionCard testId="affitti-kpi-canone">
          <div className="text-[10px] uppercase text-[#6B7280] tracking-wider">Canone mensile atteso</div>
        ...
        <SectionCard testId="affitti-kpi-incassato">
          <div className="text-[10px] uppercase text-[#6B7280] tracking-wider">Incassato (mese)</div>
          <div className="font-display text-3xl font-bold tabular mt-1 text-[#34D399]">{formatEur(incassati)}</div>
        </SectionCard>
        <SectionCard testId="affitti-kpi-occupazione">
          <div className="text-[10px] uppercase text-[#6B7280] tracking-wider">Tasso occupazione</div>
          <div className="font-display text-3xl font-bold tabular mt-1">{occupazione}%</div>
        </SectionCard>
        <SectionCard testId="affitti-kpi-morosita">
          <div className="text-[10px] uppercase text-[#6B7280] tracking-wider">Morosità</div>
          <div className="font-display text-3xl font-bold tabular mt-1 text-[#F87171]">1</div>
          <div className="text-xs text-[#9CA3AF] mt-0.5">contratto in ritardo</div>
        </SectionCard>
      </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {contratti.map(c => {
              ...
              return (
                <tr key={c.id} className="border-b border-[#212B36] last:border-0">
                  <td className="py-3 font-medium mono text-[#9CA3AF]">{c.id}</td>
                  <td className="py-3">{p?.nome || c.immobile_id}</td>
                  <td className="py-3">{c.conduttore}</td>
                  <td className="py-3 text-[#9CA3AF] text-xs">{c.inizio} → {c.fine}</td>
                  <td className="py-3 text-right tabular">{formatEur(c.canone)}</td>
                  <td className="py-3 text-right tabular text-[#9CA3AF]">{formatEur(c.deposito)}</td>
                  <td className="py-3">
                    <span className={`text-xs px-2 py-0.5 rounded-full border ${c.stato === "attivo" ? "border-[rgba(132,204,22,0.3)] text-[#A3E635] bg-[rgba(132,204,22,0.15)]" : "border-[rgba(245,158,11,0.3)] text-[#FBBF24] bg-[rgba(245,158,11,0.15)]"}`}>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {incassi.map(i => {
              ...
              return (
                <tr key={i.contratto_id} className="border-b border-[#212B36] last:border-0">
                  ...
                  <td className="py-3 text-[#9CA3AF] text-xs">{i.data || "—"}</td>
                  <td className="py-3">
