    ...
    <Layout title="Mappa Patrimonio" subtitle="Distribuzione geografica · click su un marker per i dettagli">
      <div className="grid grid-cols-1 xl:grid-cols-4 gap-4">
        <div className="xl:col-span-3">
          <div className="relative bg-[#FFFFFF] border border-[#E2E8F0] rounded-xl overflow-hidden grid-bg" style={{ aspectRatio: "16/11" }}>
            {/* Background: stylized italy outline using radial */}
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
              <div className="text-[200px] font-display font-black text-[#FFFFFF] select-none">IT</div>
            </div>
            ...
            {properties.map((p, idx) => {
              ...
              return (
                ...
                >
                  <div
                    className="w-5 h-5 rounded-full border-2 border-[#FFFFFF] shadow-lg flex items-center justify-center hover:scale-150 transition-transform"
                    style={{ background: color, boxShadow: `0 0 12px ${color}80` }}
                  >
                    <div className="w-1.5 h-1.5 bg-white rounded-full" />
                  </div>
                  <div className="absolute left-1/2 -translate-x-1/2 mt-2 opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity bg-[#F8FAFC] border border-[#E2E8F0] rounded-lg px-3 py-2 whitespace-nowrap z-20">
                    <div className="text-xs font-medium text-[#0F172A]">{p.nome}</div>
                    <div className="text-[10px] text-[#475569]">{p.citta} · {p.canone_mensile ? formatEur(p.canone_mensile)+"/mese" : "Sfitto"}</div>
                  </div>
                </Link>
              );
            })}
            ...
            {/* Legend */}
            <div className="absolute bottom-4 left-4 bg-[#F8FAFC]/90 backdrop-blur border border-[#E2E8F0] rounded-lg p-3 text-[11px] space-y-1.5 z-10">
              <div className="text-[10px] uppercase tracking-widest text-[#64748B] mb-1">Legenda</div>
