    ...
    <Layout title="Lavori & Ristrutturazioni" subtitle={`${lavori.length} cantieri attivi`}>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <SectionCard testId="lavori-kpi-cantieri">
          <div className="text-[10px] uppercase text-[#64748B]">Cantieri</div>
        ...
        <SectionCard testId="lavori-kpi-speso">
          ...
          <div className={`font-display text-3xl font-bold tabular mt-1 ${totSpeso > totBudget ? "text-[#DC2626]" : ""}`}>{formatEur(totSpeso)}</div>
        </SectionCard>
        <SectionCard testId="lavori-kpi-fuori">
          <div className="text-[10px] uppercase text-[#64748B]">Fuori budget</div>
          <div className="font-display text-3xl font-bold tabular mt-1 text-[#DC2626]">{fuoriBudget.length}</div>
        </SectionCard>
      </div>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-3 mb-2">
                    <div>
                      ...
                      <div className="text-xs text-[#475569]">{p?.nome} · Impresa: {l.impresa}</div>
                      <div className="text-[11px] text-[#64748B] mt-1">Inizio {l.inizio} · Fine prevista {l.fine_prevista}</div>
                    </div>
                    {overBudget ? (
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-3">
                    <div>
                      <div className="text-[10px] uppercase text-[#64748B]">Budget</div>
                      <div className="text-sm tabular font-medium">{formatEur(l.budget)}</div>
                    </div>
                    <div>
                      <div className="text-[10px] uppercase text-[#64748B]">Speso</div>
                      <div className={`text-sm tabular font-medium ${overBudget ? "text-[#DC2626]" : ""}`}>{formatEur(l.speso)}</div>
                    </div>
                    <div>
                      <div className="text-[10px] uppercase text-[#64748B]">Residuo</div>
                      <div className={`text-sm tabular font-medium ${remaining < 0 ? "text-[#DC2626]" : "text-[#059669]"}`}>{formatEur(remaining)}</div>
                    </div>
                    <div>
                      <div className="text-[10px] uppercase text-[#64748B]">Avanzamento</div>
                      <div className="text-sm tabular font-medium">{l.avanzamento}%</div>
                    </div>
                  </div>

                  <div className="mt-3">
                    <Progress value={l.avanzamento} className="h-2 bg-[#F8FAFC]" />
                  </div>
                </div>
              </div>
            </SectionCard>
