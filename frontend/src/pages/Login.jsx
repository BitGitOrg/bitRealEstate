...
export default function Login() {
  ...
  return (
    <div className="min-h-screen flex">
      {/* Left panel — branding */}
      <div className="hidden lg:flex w-1/2 relative overflow-hidden bg-[#0A0E14] border-r border-[#212B36]">
        ...
        <div className="relative z-10 p-12 flex flex-col justify-between w-full">
          ...
          <div>
            <h1 className="font-display text-4xl lg:text-5xl font-bold tracking-tight leading-[1.05] text-[#F3F4F6]">
              Trasforma il tuo<br />
              patrimonio in un<br />
              <span className="text-[#0066FF]">portafoglio di investimento.</span>
            </h1>
            <p className="text-sm text-[#9CA3AF] mt-6 max-w-md leading-relaxed">
            ...
            <div className="mt-8 grid grid-cols-3 gap-4 max-w-md">
              {[
                ...
                <div key={s.l} className="border-l border-[#212B36] pl-3">
                  <div className="font-display text-xl font-bold text-[#F3F4F6] tabular">{s.v}</div>
      ...
      {/* Right panel — form */}
      <div className="flex-1 flex items-center justify-center p-8 bg-[#080C11]">
        <div className="w-full max-w-md">
          ...
          <p className="text-sm text-[#9CA3AF] mt-2">Inserisci le tue credenziali per entrare nel pannello.</p>

          <form onSubmit={submit} className="mt-8 space-y-4" data-testid="login-form">
            <div>
              <label className="text-xs uppercase tracking-wider text-[#9CA3AF] font-medium">Email</label>
              <div className="mt-1.5 relative">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-3 py-2.5 bg-[#11171F] border border-[#212B36] rounded-lg text-sm focus:border-[#0066FF] outline-none transition-colors"
                  placeholder="nome@azienda.it"
                />
              </div>
            </div>
            <div>
              <label className="text-xs uppercase tracking-wider text-[#9CA3AF] font-medium">Password</label>
              <div className="mt-1.5 relative">
          ...
          {demos.length > 0 && (
            <div className="mt-8 pt-6 border-t border-[#212B36]">
              <div className="text-[10px] uppercase tracking-widest text-[#6B7280] mb-3">Account demo (click per usare)</div>
              <div className="space-y-1.5">
                {demos.map((d) => (
                  <button
                    ...
                    className="w-full flex items-center justify-between px-3 py-2 rounded-lg border border-[#212B36] hover:border-[#334155] hover:bg-[#11171F] transition-colors text-left"
                  >
                    <div>
                      <div className="text-xs text-[#F3F4F6]">{d.name}</div>
                      <div className="text-[10px] text-[#6B7280]">{d.email}</div>
                    </div>
                    <span className="text-[10px] uppercase tracking-wider text-[#60A5FA] border border-[rgba(0,102,255,0.3)] bg-[rgba(0,102,255,0.1)] rounded-full px-2 py-0.5">{d.role}</span>
                  </button>
                ))}
              </div>
            </div>
