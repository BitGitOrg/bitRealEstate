        ...
        <SectionCard testId="cr-kpi-ricavi">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-lg bg-[rgba(16,185,129,0.15)] border border-[rgba(16,185,129,0.3)] flex items-center justify-center">
              <ArrowUpRight size={18} className="text-[#34D399]"/>
            </div>
            <div>
              <div className="text-[10px] uppercase text-[#6B7280]">Ricavi (60 gg)</div>
              <div className="font-display text-2xl font-bold tabular text-[#34D399]">{formatEur(totRicavi)}</div>
            </div>
          </div>
        </SectionCard>
        <SectionCard testId="cr-kpi-costi">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-lg bg-[rgba(239,68,68,0.15)] border border-[rgba(239,68,68,0.3)] flex items-center justify-center">
              <ArrowDownRight size={18} className="text-[#F87171]"/>
            </div>
            <div>
              <div className="text-[10px] uppercase text-[#6B7280]">Costi (60 gg)</div>
              <div className="font-display text-2xl font-bold tabular text-[#F87171]">{formatEur(totCosti)}</div>
            </div>
          </div>
        </SectionCard>
        <SectionCard testId="cr-kpi-saldo">
          <div className="text-[10px] uppercase text-[#6B7280]">Saldo netto</div>
          <div className={`font-display text-3xl font-bold tabular mt-1 ${totRicavi - totCosti >= 0 ? "text-[#34D399]" : "text-[#F87171]"}`}>{formatEur(totRicavi - totCosti)}</div>
        </SectionCard>
      </div>
      ...
      <SectionCard
        ...
        action={
          <div className="flex items-center gap-2">
            <select data-testid="cr-filter-tipo" value={tipo} onChange={e => setTipo(e.target.value)} className="bg-[#080C11] border border-[#212B36] text-xs rounded-lg px-3 py-1.5 outline-none">
              <option value="tutti">Tutti</option>
              <option value="ricavo">Ricavi</option>
              <option value="costo">Costi</option>
            </select>
            <button className="inline-flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg border border-[#212B36] hover:bg-[#080C11]"><Upload size={12}/>Import</button>
            ...
            {filtered.map(m => {
              ...
              return (
                <tr key={m.id} className="border-b border-[#212B36] last:border-0">
                  <td className="py-3 text-[#9CA3AF] text-xs">{m.data}</td>
                  <td className="py-3">
                    <span className={`text-xs px-2 py-0.5 rounded-full border ${m.tipo === "ricavo" ? "border-[rgba(16,185,129,0.3)] bg-[rgba(16,185,129,0.15)] text-[#34D399]" : "border-[rgba(239,68,68,0.3)] bg-[rgba(239,68,68,0.15)] text-[#F87171]"}`}>
                      {m.categoria}
                    </span>
                  </td>
                  <td className="py-3">{m.descrizione}</td>
                  <td className="py-3 text-[#9CA3AF]">{p?.nome || "Generale"}</td>
                  <td className={`py-3 text-right tabular font-medium ${m.tipo === "ricavo" ? "text-[#34D399]" : "text-[#F87171]"}`}>
