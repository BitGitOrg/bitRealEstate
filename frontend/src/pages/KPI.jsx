...
const RankRow = ({ p, value, suffix = "" }) => (
  <Link to={`/immobile/${p.id}`} className="flex items-center gap-3 p-2 rounded-lg hover:bg-[#F8FAFC] transition-colors" data-testid={`rank-${p.id}`}>
    ...
    <div className="text-right tabular text-sm">
      <span className={p.cash_flow_mensile >= 0 ? "text-[#059669]" : "text-[#DC2626]"}>{value}{suffix}</span>
    </div>
  </Link>
);
    ...
    <Layout title="KPI & Rendimenti" subtitle="Indicatori di performance del portafoglio">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <SectionCard testId="kpi-portfolio-score" title="Portfolio Score">
          <div className="flex flex-col items-center py-2">
            ...
            <div className="text-xs text-[#475569] text-center mt-3">Salute complessiva del patrimonio (media ponderata 0-100)</div>
          </div>
        </SectionCard>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <SectionCard testId="kpi-card-medio">
          ...
          <div className="font-display text-2xl font-bold tabular mt-1 text-[#059669]">{portfolioKPI.rendimento_medio_netto}%</div>
        </SectionCard>
        <SectionCard testId="kpi-card-roi">
          <div className="text-[10px] uppercase text-[#64748B]">ROI medio</div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <SectionCard title="Da monitorare" testId="kpi-rank-worst" action={<AlertTriangle size={14} className="text-[#DC2626]"/>}>
